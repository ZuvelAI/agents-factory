...
if command -v docker >/dev/null 2>&1 && docker sbom --help >/dev/null 2>&1; then
  docker sbom --format spdx-json agents-factory-backend:test >"$output_dir/backend.spdx.json"
  docker sbom --format spdx-json agents-factory-control-plane:test >"$output_dir/control-plane.spdx.json"
else
  tool_dir=$(mktemp -d)
