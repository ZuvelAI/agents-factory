...
with predicates as (
  ...
  values
    ...
    ('audit_events', 'audit_events_retention_delete', 'PERMISSIVE', 'agents_factory_retention', 'DELETE', '((tenant_id=(NULLIF((SELECTcurrent_setting(''app.tenant_id''::text,true)AScurrent_setting),''''::text))::uuid)AND(occurred_at<agents_factory_private.retention_cutoff(''action''::text)))', '<null>'),
    ('audit_events', 'audit_events_retention_insert', 'PERMISSIVE', 'agents_factory_retention', 'INSERT', '<null>', '@tenant_id'),
    ('audit_events', 'audit_events_retention_read', 'PERMISSIVE', 'agents_factory_retention', 'SELECT', '@tenant_id', '<null>'),
    ...
    ('outbox_jobs', 'outbox_jobs_retention_read', 'PERMISSIVE', 'agents_factory_retention', 'SELECT', '@tenant_id', '<null>'),
